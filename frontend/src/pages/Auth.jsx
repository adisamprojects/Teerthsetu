...
export default function Auth() {
  ...
  return (
    <div className="min-h-screen bg-slate-50 dark:bg-[#1A1A2E] text-slate-900 dark:text-white flex items-center justify-center p-6 relative transition-colors duration-300">
      ...
      >
        ...
        <div className="w-full md:w-7/12 p-8 flex flex-col justify-center">
          ...
          <button 
            type="button"
            className={`flex-1 py-3 rounded-lg font-semibold text-sm transition-all duration-300 flex items-center justify-center gap-2 ${role === 'devotee' ? 'bg-saffron text-slate-900 dark:text-white shadow-lg' : 'text-slate-600 dark:text-slate-400 hover:text-slate-900 dark:text-white'}`}
            onClick={() => setRole('devotee')}
          >
            <User className="h-4 w-4" /> Devotee Portal
          </button>
          <button 
            type="button"
            className={`flex-1 py-3 rounded-lg font-semibold text-sm transition-all duration-300 flex items-center justify-center gap-2 ${role === 'admin' ? 'bg-emerald-600 text-slate-900 dark:text-white shadow-lg' : 'text-slate-600 dark:text-slate-400 hover:text-slate-900 dark:text-white'}`}
            onClick={() => setRole('admin')}
          >
            <Shield className="h-4 w-4" /> Temple Admin
          </button>
        </div>
        ...
        <form onSubmit={handleSubmit} className="flex flex-col gap-4">
          <AnimatePresence mode="wait">
            {isLogin ? (
              ...
              >
                ...
                <div className="relative">
                  <MapPin className="absolute left-4 top-3.5 h-5 w-5 text-slate-600 dark:text-slate-400" />
                  <input 
                    type="text" 
                    name="address" 
                ...
                <div className="relative">
                  <Key className="absolute left-4 top-3.5 h-5 w-5 text-slate-600 dark:text-slate-400" />
                  <input 
                    type="text" 
                    name="aadhaar" 
                    placeholder="Aadhaar Card Number (Optional)" 
                    className="w-full bg-slate-100 dark:bg-white/5 border border-slate-200 dark:border-white/10 rounded-xl pl-12 pr-4 py-3 text-slate-900 dark:text-white focus:outline-none focus:border-saffron focus:ring-1 focus:ring-saffron transition-all"
                ...
                <div className="relative">
                  <UserCheck className="absolute left-4 top-3.5 h-5 w-5 text-slate-600 dark:text-slate-400" />
                  <input 
                    type="text" 
          ...
          <button 
            type="submit" 
            className={`w-full py-3.5 rounded-xl font-bold text-lg mt-4 shadow-lg transition-all duration-300 hover:-translate-y-0.5 ${role === 'admin' ? 'bg-emerald-600 hover:bg-emerald-500 shadow-emerald-950/30' : 'bg-saffron hover:bg-[#e85a28] shadow-saffron/30'}`}
          >
        ...
        {role === 'devotee' && (
          <p className="text-center text-slate-600 dark:text-slate-400 mt-6 text-sm">
            {isLogin ? "New to TeerthSethu? " : "Already registered? "}
            <span 
              className="text-gold font-semibold cursor-pointer hover:underline" 
              onClick={() => setIsLogin(!isLogin)}
            >
              {isLogin ? 'Register details here' : 'Login to dashboard'}
            </span>
          </p>
        )}
        </div>
